...
uint16_t adc_value; // uint8_t에서 uint16_t로 변경
uint8_t adc_value_1000=1, adc_value_100=1, adc_value_10=1, adc_value_1=1;
...
int main(void) {
    ...
    while (1) {
        //adc_value = HAL_ADC_GetValue(&hadc1); // adc_value 값을 읽어옴
        if (stop_flag == 0) {
            adc_value_1000 = 1;
            adc_value_100 = 1;
            adc_value_10 = 1;
        ...
        display_fnd(number[adc_value_1000], 3, 5);
        display_fnd(number[adc_value_100], 2, 5);
        display_fnd(number[adc_value_10], 1, 5);
        display_fnd(number[adc_value_1], 0, 5);
    }
}
